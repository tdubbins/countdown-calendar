...
spec = importlib.util.spec_from_file_location("json_db", os.path.join(backend_path, "app", "utils", "json_db.py"))
json_db_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(json_db_module)
JSONDatabase = json_db_module.JSONDatabase

# Create calendars_db instance directly
data_path = os.path.join(backend_path, 'data', 'calendars.json')
calendars_db = JSONDatabase(data_path)
